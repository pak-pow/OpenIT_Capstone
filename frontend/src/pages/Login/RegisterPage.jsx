...
import {
  ...
  BookOpen, Home, TrendingUp, GraduationCap, Heart,
} from 'lucide-react';
...
const getCourseOptions = (educationLevel) => {
  switch (educationLevel) {
    case 'jhs':        return JHS_TRACKS;
    case 'shs':        return SHS_STRANDS;
    case 'college':    return COURSES;
    case 'vocational': return VOCATIONAL_COURSES;
    default:           return [];
  }
};
...
const getCourseLabel = (educationLevel) => {
  switch (educationLevel) {
    case 'jhs':        return 'Program / Track';
    case 'shs':        return 'Strand';
    case 'college':    return 'Course / Program';
    case 'vocational': return 'TESDA Qualification';
...
const getYearLabel = (educationLevel) => {
  switch (educationLevel) {
    case 'jhs':        return 'Grade Level';
    case 'shs':        return 'Grade Level';
    case 'vocational': return 'Year Level';
...
const RegisterPage = ({ onNavigateLogin }) => {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'educationLevel') {
      setForm((prev) => ({ ...prev, educationLevel: value, yearLevel: '', course: '' }));
  ...
  const validateStep2 = () => {
    ...
    if (!form.yearLevel)          errs.yearLevel       = 'Please select your grade/year level.';
    ...
    if (!form.incomeBracket)      errs.incomeBracket   = 'Please select a household income bracket.';
  ...
  const gradeLevels   = form.educationLevel ? GRADE_LEVELS[form.educationLevel] : [];

  return (
    <div className="auth-page">
      <div className={`auth-card auth-card-wide ${step === 2 ? 'auth-card-scroll' : ''}`}>
        ...
        {step === 1 && (
          <form className="auth-form compact" onSubmit={handleNextStep}>
            <div className="name-row">
              <div className="form-group">
                ...
                <div className="input-wrapper">
                  ...
                  <input id="firstName" className="form-input" type="text" name="firstName"
              ...
              <div className="form-group">
                ...
                <div className="input-wrapper">
                  <User size={18} className="input-icon" />
                  <input id="lastName" className="form-input" type="text" name="lastName"
                    placeholder="Dela Cruz" value={form.lastName} onChange={handleChange} />
                </div>
            ...
            <div className="form-group">
              ...
              <div className="input-wrapper">
                <Lock size={18} className="input-icon" />
                <input id="confirmPassword" className="form-input" type="password"
        ...
        {step === 2 && (
          <form className="auth-form" onSubmit={handleSubmit}>
            ...
            <div className="profile-hint">
              <TrendingUp size={16} />
              <span>
                This profile powers your <strong>Smart Match Score</strong> — we'll rank
                scholarships by how well you qualify. Be as accurate as possible.
              </span>
            </div>
            ...
            <div className="form-group">
              ...
              {errors.educationLevel && <span className="form-error">{errors.educationLevel}</span>}
            </div>

            {/* GWA + Year Level (side by side) */}
            <div className="name-row">
              <div className="form-group">
                <label className="form-label" htmlFor="gwa">GWA</label>
                <input id="gwa" className="form-input" type="number" name="gwa"
                  placeholder="e.g. 1.50" min="1.00" max="5.00" step="0.01"
                  value={form.gwa} onChange={handleChange} />
                <span className="form-hint">1.00 = highest · 5.00 = lowest</span>
              ...
              <div className="form-group">
                ...
                <div className="input-wrapper">
                  ...
                  <select id="yearLevel" className="form-input form-select"
                ...
                {errors.yearLevel && <span className="form-error">{errors.yearLevel}</span>}
              </div>
            </div>
            ...
            <div className="form-group">
              ...
              <div className="input-wrapper">
                <BookOpen size={18} className="input-icon" />
                <select id="course" className="form-input form-select"
                  name="course" value={form.course} onChange={handleChange}
                  disabled={!form.educationLevel}>
                  <option value="">
                    {form.educationLevel ? `Select ${getCourseLabel(form.educationLevel).toLowerCase()}` : 'Select education level first'}
                  </option>
                  {courseOptions.map((c) => (
            ...
            <div className="form-group">
              <label className="form-label" htmlFor="barangay">Barangay of Residence</label>
              <div className="input-wrapper">
                <Home size={18} className="input-icon" />
                <select id="barangay" className="form-input form-select"
            ...
            <div className="form-group">
              ...
              <div className="peso-prefix-wrapper">
                <span className="peso-prefix">₱</span>
                <select id="incomeBracket" className="form-input form-select"
                  name="incomeBracket" value={form.incomeBracket} onChange={handleChange}>
              ...
              {errors.incomeBracket && <span className="form-error">{errors.incomeBracket}</span>}
            </div>
            ...
            <div className="eligibility-checks">
              <label className="check-item">
                <input type="checkbox" name="isPwd" checked={form.isPwd} onChange={handleChange} />
                <div className="check-text">
                  <span className="check-title">I have a PWD (Person with Disability) ID</span>
                  <span className="check-sub">Issued by a local government unit or NCDA</span>
                </div>
              </label>

              <label className="check-item">
                <input type="checkbox" name="isSoloParent" checked={form.isSoloParent} onChange={handleChange} />
                <div className="check-text">
                  <span className="check-title">I am a solo parent or a child of a solo parent</span>
                  <span className="check-sub">Under Republic Act 8972 (Solo Parents' Welfare Act)</span>
                </div>
              </label>

              <label className="check-item">
                <input type="checkbox" name="isIndigenous" checked={form.isIndigenous} onChange={handleChange} />
                <div className="check-text">
                  <span className="check-title">I belong to an Indigenous Peoples (IP) community</span>
                  <span className="check-sub">Recognized by the National Commission on Indigenous Peoples (NCIP)</span>
                </div>
              </label>
            </div>
        ...
        <div className="auth-footer">
          ...
          <button className="auth-link" onClick={onNavigateLogin}>Sign In</button>
        </div>
      </div>
    </div>
  );
